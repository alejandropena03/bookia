  ...
  return (
    <div className="space-y-3">
      {conversations.map((conv) => {
        ...
        return (
          <Link key={conv.id} href={`/conversations/${conv.id}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors group">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-sm font-medium text-[#0F172A]">{conv.contact_name}</span>
                <Badge className={`text-xs py-0 px-1.5 border-0 ${CANAL_STYLES[conv.canal] ?? "bg-gray-100 text-gray-600"}`}>{conv.canal}</Badge>
              </div>
              <p className="text-xs text-[#64748B] truncate">{lastMsg?.content?.slice(0, 60)}...</p>
            </div>
            <div className="flex flex-col items-end gap-1 shrink-0">
              ...
              <span className="text-xs text-[#94A3B8]">{formatRelativeTime(conv.updated_at)}</span>
            </div>
          </Link>
        )
      })}
